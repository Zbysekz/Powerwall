...
def main(args):
    ...
    # logging
    logging.basicConfig(level=loglevel, format='%(asctime)-8s [%(levelname)s]: %(message)s',
                        datefmt='%H:%M:%S')
    ...
    return serve(options.esp_ip, options.host_ip, options.esp_port, options.host_port, options.auth,
                 options.image, command)

# end main

if __name__ == '__main__':
    main([sys.argv[0], "-i", "192.168.0.15", "-f",
          "/media/D/Data/ArduinoProjects/esp_epever_powerwall/build/esp8266.esp8266.nodemcuv2/esp_epever_powerwall.ino.bin",
          "-d"])  #main(sys.argv)
    #raw_input();
